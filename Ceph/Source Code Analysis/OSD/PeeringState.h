...
public:
    struct PeeringListener : public EpochSource {
        ...
        virtual void send_cluster_message(int osd,
                                          MessageRef m,
                                          epoch_t epoch,
        ...
        virtual std::pair<ghobject_t, bool> do_delete_work(ObjectStore::Transaction &t,
                                                           ghobject_t _next) = 0;
        ...
        virtual void log_state_exit(const char *state_name,
                                    utime_t enter_time,
                                    uint64_t events,
                                    utime_t event_dur) = 0;
...
public:
    ...
    TrivialEvent(Initialize) TrivialEvent(GotInfo) TrivialEvent(NeedUpThru) TrivialEvent(Backfilled)
        TrivialEvent(LocalBackfillReserved) TrivialEvent(RejectTooFullRemoteReservation)
            TrivialEvent(RequestBackfill) TrivialEvent(RemoteRecoveryPreempted) TrivialEvent(
                RemoteBackfillPreempted) TrivialEvent(BackfillTooFull) TrivialEvent(RecoveryTooFull)
                                            ...
                                            TrivialEvent(DeleteStart) TrivialEvent(DeleteSome)
                                                ...
                                                TrivialEvent(SetForceRecovery)
                                                    TrivialEvent(UnsetForceRecovery)
                                                        TrivialEvent(SetForceBackfill) TrivialEvent(
                                                            UnsetForceBackfill)
...
private:
    ...
    public:
        ...
        PeeringMachine(PeeringState *state,
                       CephContext *cct,
                       spg_t spgid,
                       DoutPrefixProvider *dpp,
                       PeeringListener *pl,
    ...
    struct Crashed
        : boost::statechart::state<Crashed, PeeringMachine>
        , NamedState {
    ...
    struct Initial
        : boost::statechart::state<Initial, PeeringMachine>
        , NamedState {
    ...
    struct Reset
        : boost::statechart::state<Reset, PeeringMachine>
        , NamedState {
    ...
    struct Started
        : boost::statechart::state<Started, PeeringMachine, Start>
        , NamedState {
    ...
    struct Primary
        : boost::statechart::state<Primary, Started, Peering>
        , NamedState {
    ...
    struct WaitActingChange
        : boost::statechart::state<WaitActingChange, Primary>
        , NamedState {
    ...
    struct Activating;
    struct Active
        : boost::statechart::state<Active, Primary, Activating>
        , NamedState {
    ...
    struct Clean
        : boost::statechart::state<Clean, Active>
        , NamedState {
        typedef boost::mpl::list<
    ...
    struct Recovered
        : boost::statechart::state<Recovered, Active>
        , NamedState {
        typedef boost::mpl::list<
    ...
    struct Backfilling
        : boost::statechart::state<Backfilling, Active>
        , NamedState {
        typedef boost::mpl::list<
            boost::statechart::custom_reaction<Backfilled>,
            boost::statechart::custom_reaction<DeferBackfill>,
    ...
    struct NotBackfilling
        : boost::statechart::state<NotBackfilling, Active>
        , NamedState {
        typedef boost::mpl::list<
            boost::statechart::custom_reaction<QueryUnfound>,
    ...
    struct NotRecovering
        : boost::statechart::state<NotRecovering, Active>
        , NamedState {
        typedef boost::mpl::list<
            boost::statechart::custom_reaction<QueryUnfound>,
            boost::statechart::transition<DoRecovery, WaitLocalRecoveryReserved>,
    ...
    struct RepRecovering
        : boost::statechart::state<RepRecovering, ReplicaActive>
        , NamedState {
        typedef boost::mpl::list<
    ...
    struct RepNotRecovering
        ...
        typedef boost::mpl::list<
            ...
            boost::statechart::custom_reaction<RemoteRecoveryReserved>,
            boost::statechart::custom_reaction<RemoteBackfillReserved>,
            boost::statechart::transition<RecoveryDone, RepNotRecovering>   // for compat with
                                                                            // pre-reservation peers
            >
            reactions;
    ...
    struct Recovering
        : boost::statechart::state<Recovering, Active>
        , NamedState {
        typedef boost::mpl::list<boost::statechart::custom_reaction<AllReplicasRecovered>,
                                 boost::statechart::custom_reaction<DeferRecovery>,
                                 boost::statechart::custom_reaction<UnfoundRecovery>,
    ...
    struct Activating
        : boost::statechart::state<Activating, Active>
        , NamedState {
        typedef boost::mpl::list<
    ...
    struct Stray
        : boost::statechart::state<Stray, Started>
        , NamedState {
        explicit Stray(my_context ctx);
        void exit();

        typedef boost::mpl::list<boost::statechart::custom_reaction<MQuery>,
                                 boost::statechart::custom_reaction<MLogRec>,
                                 boost::statechart::custom_reaction<MInfoRec>,
                                 boost::statechart::custom_reaction<ActMap>,
                                 boost::statechart::custom_reaction<RecoveryDone>,
    ...
    struct Deleting
        : boost::statechart::state<Deleting, ToDelete>
        , NamedState {
        typedef boost::mpl::list<
            boost::statechart::custom_reaction<DeleteSome>,
    ...
    struct WaitUpThru
        : boost::statechart::state<WaitUpThru, Peering>
        , NamedState {
    ...
    struct Down
        : boost::statechart::state<Down, Peering>
        , NamedState {
    ...
    struct Incomplete
        : boost::statechart::state<Incomplete, Peering>
        , NamedState {
        typedef boost::mpl::list<boost::statechart::custom_reaction<AdvMap>,
                                 boost::statechart::custom_reaction<MNotifyRec>,
                                 boost::statechart::custom_reaction<QueryUnfound>,
                                 boost::statechart::custom_reaction<QueryState>>
            reactions;
    ...
    static void calc_ec_acting(std::map<pg_shard_t, pg_info_t>::const_iterator auth_log_shard,
                               unsigned size,
    ...
    select_replicated_primary(map<pg_shard_t, pg_info_t>::const_iterator auth_log_shard,
    ...
    static void calc_replicated_acting(map<pg_shard_t, pg_info_t>::const_iterator primary_shard,
    ...
    void choose_async_recovery_ec(const std::map<pg_shard_t, pg_info_t> &all_info,
                                  const pg_info_t &auth_info,
                                  std::vector<int> *want,
                                  std::set<pg_shard_t> *async_recovery,
                                  const OSDMapRef osdmap) const;
    void choose_async_recovery_replicated(const std::map<pg_shard_t, pg_info_t> &all_info,
                                          const pg_info_t &auth_info,
                                          std::vector<int> *want,
                                          std::set<pg_shard_t> *async_recovery,
                                          const OSDMapRef osdmap) const;
    ...
    void proc_master_log(ObjectStore::Transaction &t,
                         pg_info_t &oinfo,
                         pg_log_t &&olog,
    ...
    void fulfill_info(pg_shard_t from,
...
public:
    ...
    void append_log_with_trim_to_updated(std::vector<pg_log_entry_t> &&log_entries,
     ...
     */
    void merge_new_log_entries(const mempool::osd_pglog::list<pg_log_entry_t> &entries,
                               ObjectStore::Transaction &t,
                               std::optional<eversion_t> trim_to,
                               std::optional<eversion_t> roll_forward_to);
    ...
    void recover_got(const hobject_t &oid,
                     eversion_t v,
                     bool is_delete,
                     ObjectStore::Transaction &t);
