...
void BufferedRecoveryMessages::send_info(int to,
    ...
    if (require_osd_release >= ceph_release_t::octopus) {
        send_osd_message(
            to, make_message<MOSDPGInfo2>(to_spgid, info, cur_epoch, min_epoch, lease, lease_ack));
    } else {
        send_osd_message(to,
                         make_message<MOSDPGInfo>(cur_epoch,
...
void PeeringState::update_history(const pg_history_t &new_history) {
    ...
    if (info.history.merge(new_history)) {
        ...
        if (prior_readable_until_ub != ceph::signedspan::zero()) {
            dout(20) << __func__ << " prior_readable_until_ub " << prior_readable_until_ub
                     << " (mnow " << mnow << " + " << info.history.prior_readable_until_ub << ")"
                     << dendl;
        }
    }
...
bool PeeringState::proc_replica_info(pg_shard_t from, const pg_info_t &oinfo, epoch_t send_epoch) {
    ...
    if (p != peer_info.end() && p->second.last_update == oinfo.last_update) {
        psdout(10) << " got dup osd." << from << " info " << oinfo << ", identical to ours"
                   << dendl;
        return false;
    }
...
void PeeringState::advance_map(OSDMapRef osdmap,
                               OSDMapRef lastmap,
                               vector<int> &newup,
                               int up_primary,
    ...
    psdout(10) << "handle_advance_map " << newup << "/" << newacting << " -- " << up_primary << "/"
...
void PeeringState::activate_map(PeeringCtx &rctx) {
    ...
    if (osdmap_ref->get_epoch() - last_persisted_osdmap >
        ...
        psdout(20) << __func__ << ": Dirtying info: last_persisted is " << last_persisted_osdmap
...
void PeeringState::start_peering_interval(const OSDMapRef lastmap,
    ...
    psdout(1) << __func__ << " up " << oldup << " -> " << up << ", acting " << oldacting << " -> "
              << acting << ", acting_primary " << old_acting_primary << " -> " << new_acting_primary
              << ", up_primary " << old_up_primary << " -> " << new_up_primary << ", role "
              << oldrole << " -> " << role << ", features acting " << acting_features
...
void PeeringState::on_new_interval() {
    ...
    psdout(20) << __func__ << " upacting_features 0x" << std::hex << upacting_features << std::dec
               << " from " << acting << "+" << up << dendl;

    psdout(20) << __func__
               << " checking missing set deletes flag. missing = " << get_pg_log().get_missing()
               << dendl;
    ...
    prior_readable_until_ub =
        info.history.refresh_prior_readable_until_ub(mnow, prior_readable_until_ub);
    psdout(10) << __func__ << " prior_readable_until_ub " << prior_readable_until_ub << " (mnow "
               << mnow << " + " << info.history.prior_readable_until_ub << ")" << dendl;
...
void PeeringState::init_primary_up_acting(const vector<int> &newup,
    ...
    for (uint8_t i = 0; i < acting.size(); ++i) {
        if (acting[i] != CRUSH_ITEM_NONE)
            actingset.insert(pg_shard_t(
                acting[i], pool.info.is_erasure() ? shard_id_t(i) : shard_id_t::NO_SHARD));
    }
    upset.clear();
    up = newup;
    for (uint8_t i = 0; i < up.size(); ++i) {
        if (up[i] != CRUSH_ITEM_NONE)
            upset.insert(
                pg_shard_t(up[i], pool.info.is_erasure() ? shard_id_t(i) : shard_id_t::NO_SHARD));
    }
...
static pair<epoch_t, epoch_t> get_required_past_interval_bounds(const pg_info_t &info,
                                                                epoch_t oldest_map) {
    epoch_t start = std::max(info.history.last_epoch_clean ? info.history.last_epoch_clean
...
void PeeringState::check_past_interval_bounds() const {
    ...
    if (rpib.first >= rpib.second) {
        ...
        if (past_intervals.empty()) {
            ...
            derr << info.pgid << " required past_interval bounds are"
                 << " not empty [" << rpib << ") but past_intervals " << past_intervals
                 << " is empty" << dendl;
...
void PeeringState::send_lease() {
    ...
    for (auto peer : actingset) {
        ...
        pl->send_cluster_message(
            peer.osd,
            make_message<MOSDPGLease>(epoch, spg_t(spgid.pgid, peer.shard), get_lease()),
            epoch);
    }
}
...
void PeeringState::proc_lease(const pg_lease_t &l) {
    if (!HAVE_FEATURE(upacting_features, SERVER_OCTOPUS)) {
        psdout(20) << __func__ << " no-op, upacting_features 0x" << std::hex << upacting_features
                   << std::dec << " does not include SERVER_OCTOPUS" << dendl;
        return;
    }
    if (!is_nonprimary()) {
    ...
    if (l.readable_until != ceph::signedspan::zero() && hb_stamps[0]->peer_clock_delta_ub) {
        ...
        psdout(20) << " peer_clock_delta_ub " << *hb_stamps[0]->peer_clock_delta_ub << " -> ru "
                   << ru << dendl;
    }
    ...
    if (hb_stamps[0]->peer_clock_delta_lb) {
        ...
        psdout(20) << " peer_clock_delta_lb " << *hb_stamps[0]->peer_clock_delta_lb << " -> ruub "
                   << ruub << dendl;
    } else {
...
void PeeringState::recalc_readable_until() {
    ...
    for (unsigned i = 0; i < acting.size(); ++i) {
        ...
        dout(20) << __func__ << " peer osd." << acting[i] << " ruub " << acting_readable_until_ub[i]
                 << dendl;
...
bool PeeringState::needs_recovery() const {
    ...
    for (const pg_shard_t &peer : acting_recovery_backfill) {
        ...
        if (pm->second.num_missing()) {
            psdout(10) << __func__ << " osd." << peer << " has " << pm->second.num_missing()
                       << " missing" << dendl;
            return true;
        }
    }
...
void PeeringState::reject_reservation() {
    ...
    pl->send_cluster_message(primary.osd,
                             make_message<MBackfillReserve>(MBackfillReserve::REJECT_TOOFULL,
...
void PeeringState::calc_ec_acting(map<pg_shard_t, pg_info_t>::const_iterator auth_log_shard,
                                  unsigned size,
...
public:
    ...
    osd_ord_t get_ord() const {
        return osds.empty() ? std::make_tuple(false, eversion_t()) : osds.front().first;
    }
...
void PeeringState::calc_replicated_acting_stretch(
    ...
    ss << " osd " << primary->first.osd << " primary accepted " << osd_info(primary->first.osd)
       << std::endl;
    for (auto upcand : up) {
    ...
    bucket_heap_t aheap;
    std::for_each(
        ancestors.begin(), ancestors.end(), [&](auto &anc) { aheap.push_if_nonempty(anc.second); });
...
bool PeeringState::recoverable(const vector<int> &want) const {
    ...
    for (int i = 0; i < (int)want.size(); ++i) {
        if (want[i] != CRUSH_ITEM_NONE) {
            ++num_want_acting;
            have.insert(
                pg_shard_t(want[i], pool.info.is_erasure() ? shard_id_t(i) : shard_id_t::NO_SHARD));
        }
    }
...
void PeeringState::choose_async_recovery_ec(const map<pg_shard_t, pg_info_t> &all_info,
    ...
    psdout(20) << __func__ << " result want=" << *want << " async_recovery=" << *async_recovery
               << dendl;
}
...
void PeeringState::log_weirdness() {
    if (pg_log.get_tail() != info.log_tail)
        pl->get_clog_error() << info.pgid << " info mismatch, log.tail " << pg_log.get_tail()
                             << " != info.log_tail " << info.log_tail;
    if (pg_log.get_head() != info.last_update)
        pl->get_clog_error() << info.pgid << " info mismatch, log.head " << pg_log.get_head()
                             << " != info.last_update " << info.last_update;

    if (!pg_log.get_log().empty()) {
        // sloppy check
        if ((pg_log.get_log().log.begin()->version <= pg_log.get_tail()))
            pl->get_clog_error() << info.pgid << " log bound mismatch, info (tail,head] ("
...
bool PeeringState::discover_all_missing(BufferedRecoveryMessages &rctx) {
    ...
    psdout(10) << __func__ << " " << missing.num_missing() << " missing, " << unfound << " unfound"
               << dendl;
    ...
    for (auto pg_shard : acting_recovery_backfill) {
        ...
        if (last_require_osd_release >= ceph_release_t::octopus) {
            ...
            m = make_message<MOSDPGInfo>(get_osdmap_epoch(),
                                         MOSDPGInfo::pg_list_t{pg_notify_t{pg_shard.shard,
                                                                           pg_whoami.shard,
                                                                           get_osdmap_epoch(),
                                                                           get_osdmap_epoch(),
                                                                           info,
...
void PeeringState::merge_log(ObjectStore::Transaction &t,
                             pg_info_t &oinfo,
                             pg_log_t &&olog,
                             pg_shard_t from) {
    PGLog::LogEntryHandlerRef rollbacker{pl->get_log_handler(t)};
    pg_log.merge_log(
        oinfo, std::move(olog), from, info, rollbacker.get(), dirty_info, dirty_big_info);
}
...
void PeeringState::proc_replica_log(pg_info_t &oinfo,
                                    ...
                                    pg_missing_t &&omissing,
                                    pg_shard_t from) {
    psdout(10) << "proc_replica_log for osd." << from << ": " << oinfo << " " << olog << " "
               << omissing << dendl;
    ...
    for (auto i = omissing.get_items().begin(); i != omissing.get_items().end(); ++i) {
        psdout(20) << " after missing " << i->first << " need " << i->second.need << " have "
                   << i->second.have << dendl;
    }
    peer_missing[from].claim(std::move(omissing));
}
...
void PeeringState::fulfill_log(pg_shard_t from, const pg_query_t &query, epoch_t query_epoch) {
    ...
    auto mlog =
        make_message<MOSDPGLog>(from.shard, pg_whoami.shard, get_osdmap_epoch(), info, query_epoch);
...
void PeeringState::merge_from(map<spg_t, PeeringState *> &sources,
        ...
        if (last_pg_merge_meta.source_pgid != pg_t()) {
            if (source->info.pgid.pgid != last_pg_merge_meta.source_pgid) {
                dout(10) << __func__ << " source " << source->info.pgid.pgid
                         << " doesn't match expected source pgid " << last_pg_merge_meta.source_pgid
                         << dendl;
                incomplete = true;
            }
    ...
    if (info.history.epoch_created == 0) {
        ...
        if (info.history.last_epoch_clean < pib.first) {
            psdout(10) << __func__ << " last_epoch_clean " << info.history.last_epoch_clean
                       << " < past_interval start " << pib.first << ", adjusting start backwards"
                       << dendl;
        ...
        if (info.history.last_epoch_started < info.history.same_interval_since) {
            ...
            info.history.same_up_since =
                std::min(info.history.same_up_since, info.history.same_interval_since);
...
void PeeringState::update_calc_stats() {
    ...
    if (info.stats.stats.sum.num_objects < 0) {
        psdout(0) << __func__ << " negative num_objects = " << info.stats.stats.sum.num_objects
    ...
    if ((is_remapped() || is_undersized() || !is_clean()) && (is_peered() || is_activating())) {
        psdout(20) << __func__ << " actingset " << actingset << " upset " << upset
                   << " acting_recovery_backfill " << acting_recovery_backfill << dendl;
...

        {
            ...
            psdout(20) << __func__ << " shard " << pg_whoami << " primary objects " << num_objects
        ...
        for (auto &peer : peer_info) {
            ...
            psdout(20) << __func__ << " shard " << peer.first << " objects " << peer_num_objects
                       << " missing " << missing << dendl;
        }
        ...
        psdout(20) << __func__ << " object_location_counts " << info.stats.object_location_counts
                   << dendl;
            ...
            for (auto &sml : missing_loc.get_missing_by_count()) {
                for (auto &ml : sml.second) {
                    ...
                    if (sml.first == shard_id_t::NO_SHARD) {
                        ...
                        psdout(20) << __func__ << " shard " << sml.first << " ml " << ml.second
...
std::optional<pg_stat_t> PeeringState::prepare_stats_for_publish(
    ...
    if (info.stats.stats.sum.num_scrub_errors) {
        psdout(10) << __func__ << " inconsistent due to " << info.stats.stats.sum.num_scrub_errors
                   << " scrub errors" << dendl;
        state_set(PG_STATE_INCONSISTENT);
    } else {
...
void PeeringState::init(int role,
                        ...
                        bool backfill,
                        ObjectStore::Transaction &t) {
    psdout(10) << "init role " << role << " up " << newup << " acting " << newacting << " history "
               << history << " past_intervals " << pi << dendl;
...
void PeeringState::update_stats(std::function<bool(pg_history_t &, pg_stat_t &)> f,
                                ObjectStore::Transaction *t) {
...
bool PeeringState::append_log_entries_update_missing(
    ...
    bool invalidate_stats =
        pg_log.append_new_log_entries(info.last_backfill, entries, rollbacker.get());
...
void PeeringState::append_log(vector<pg_log_entry_t> &&logv,
                              eversion_t trim_to,
    ...
    psdout(10) << __func__ << " approx pg log length =  " << pg_log.get_log().approx_size()
               << dendl;
    ...
    if (!transaction_applied || async)
        psdout(10) << __func__ << " " << pg_whoami << " is async_recovery or backfill target"
                   << dendl;
...
void PeeringState::recover_got(const hobject_t &oid,
    ...
    if (pg_log.get_log().log.empty()) {
        ...
        psdout(10) << "last_complete now " << info.last_complete << " log.complete_to at end"
                   << dendl;
...
void PeeringState::pre_submit_op(const hobject_t &hoid,
    ...
    if (requires_missing_loc) {
        for (auto &&entry : logv) {
            psdout(30) << __func__
                       << " missing_loc before: " << missing_loc.get_locations(entry.soid) << dendl;
            ...
            psdout(30) << __func__
                       << " missing_loc after: " << missing_loc.get_locations(entry.soid) << dendl;
        }
    }
}
...
void PeeringState::calc_trim_to() {
    ...
    if (limit != eversion_t() && limit != pg_trim_to && pg_log.get_log().approx_size() > target) {
        size_t num_to_trim =
            std::min(pg_log.get_log().approx_size() - target, cct->_conf->osd_pg_log_trim_max);
...
void PeeringState::calc_trim_to_aggressive() {
    ...
    eversion_t limit =
        std::min({pg_log.get_head(), pg_log.get_can_rollback_to(), last_update_ondisk});
...
void PeeringState::update_complete_backfill_object_stats(const hobject_t &hoid,
...
void PeeringState::prepare_backfill_for_missing(const hobject_t &soid,
                                                const eversion_t &version,
...
#undef dout_prefix
#define dout_prefix \
    (context<PeeringMachine>().dpp->gen_prefix(*_dout) << "state<" << get_state_name() << ">: ")
#undef psdout
...
boost::statechart::result PeeringState::Started::react(const AdvMap &advmap) {
    ...
    if (ps->should_restart_peering(advmap.up_primary,
...
boost::statechart::result PeeringState::Reset::react(const ActMap &) {
    ...
    if (ps->should_send_notify() && ps->get_primary().osd >= 0) {
        ps->info.history.refresh_prior_readable_until_ub(pl->get_mnow(),
...
boost::statechart::result PeeringState::Backfilling::react(const DeferBackfill &c) {
    ...
    pl->schedule_event_after(std::make_shared<PGPeeringEvent>(
...
boost::statechart::result PeeringState::Backfilling::react(
    const RemoteReservationRevokedTooFull &) {
    DECLARE_LOCALS;
...
PeeringState::WaitLocalBackfillReserved::WaitLocalBackfillReserved(my_context ctx)
    ...
    pl->request_local_background_io_reservation(
        ...
        std::make_unique<PGPeeringEvent>(
            ps->get_osdmap_epoch(), ps->get_osdmap_epoch(), LocalBackfillReserved()),
        std::make_unique<PGPeeringEvent>(
...
boost::statechart::result PeeringState::RepWaitRecoveryReserved::react(
    ...
    pl->send_cluster_message(
        ps->primary.osd,
        make_message<MRecoveryReserve>(MRecoveryReserve::GRANT,
...
boost::statechart::result PeeringState::RepNotRecovering::react(const RequestBackfillPrio &evt) {
    ...
    if (!pl->try_reserve_recovery_space(evt.primary_num_bytes, evt.local_num_bytes)) {
        ...
        pl->request_remote_recovery_reservation(
            evt.priority,
            std::make_unique<PGPeeringEvent>(
                pl->get_osdmap_epoch(), pl->get_osdmap_epoch(), RemoteBackfillReserved()),
            std::move(preempt));
    }
...
boost::statechart::result PeeringState::RepNotRecovering::react(const RequestRecoveryPrio &evt) {
    ...
    if (HAVE_FEATURE(ps->upacting_features, RECOVERY_RESERVATION_2)) {
        ...
        preempt = std::make_unique<PGPeeringEvent>(
            ps->get_osdmap_epoch(), ps->get_osdmap_epoch(), RemoteRecoveryPreempted());
    }

    pl->request_remote_recovery_reservation(
        prio,
        std::make_unique<PGPeeringEvent>(
            ps->get_osdmap_epoch(), ps->get_osdmap_epoch(), RemoteRecoveryReserved()),
        std::move(preempt));
...
boost::statechart::result PeeringState::RepWaitBackfillReserved::react(
    ...
    pl->send_cluster_message(
        ps->primary.osd,
        make_message<MBackfillReserve>(MBackfillReserve::GRANT,
                                       spg_t(ps->info.pgid.pgid, ps->primary.shard),
                                       ps->get_osdmap_epoch()),
        ps->get_osdmap_epoch());
    return transit<RepRecovering>();
...
boost::statechart::result PeeringState::RepRecovering::react(const RemoteRecoveryPreempted &) {
    ...
    pl->send_cluster_message(
        ps->primary.osd,
        make_message<MRecoveryReserve>(MRecoveryReserve::REVOKE,
                                       spg_t(ps->info.pgid.pgid, ps->primary.shard),
                                       ps->get_osdmap_epoch()),
        ps->get_osdmap_epoch());
    return discard_event();
...
boost::statechart::result PeeringState::RepRecovering::react(const BackfillTooFull &) {
    ...
    pl->send_cluster_message(
        ps->primary.osd,
        make_message<MBackfillReserve>(MBackfillReserve::REVOKE_TOOFULL,
                                       spg_t(ps->info.pgid.pgid, ps->primary.shard),
                                       ps->get_osdmap_epoch()),
        ps->get_osdmap_epoch());
    return discard_event();
...
boost::statechart::result PeeringState::RepRecovering::react(const RemoteBackfillPreempted &) {
    ...
    pl->send_cluster_message(
        ps->primary.osd,
        make_message<MBackfillReserve>(MBackfillReserve::REVOKE,
                                       spg_t(ps->info.pgid.pgid, ps->primary.shard),
                                       ps->get_osdmap_epoch()),
        ps->get_osdmap_epoch());
    return discard_event();
...
PeeringState::WaitLocalRecoveryReserved::WaitLocalRecoveryReserved(my_context ctx)
    ...
    pl->request_local_background_io_reservation(
        ...
        std::make_unique<PGPeeringEvent>(
            ps->get_osdmap_epoch(), ps->get_osdmap_epoch(), LocalRecoveryReserved()),
        std::make_unique<PGPeeringEvent>(
            ps->get_osdmap_epoch(), ps->get_osdmap_epoch(), DeferRecovery(0.0)));
    pl->publish_stats_to_osd();
}

boost::statechart::result PeeringState::WaitLocalRecoveryReserved::react(
    const RecoveryTooFull &evt) {
    DECLARE_LOCALS;
...
void PeeringState::Recovering::release_reservations(bool cancel) {
    ...
    for (auto i = context<Active>().remote_shards_to_reserve_recovery.begin();
        ...
        pl->send_cluster_message(i->osd,
                                 make_message<MRecoveryReserve>(MRecoveryReserve::RELEASE,
...
PeeringState::Recovered::Recovered(my_context ctx)
    ...
    if (context<Active>().all_replicas_activated && ps->async_recovery_targets.empty())
        post_event(GoClean());
}
...
boost::statechart::result PeeringState::Active::react(const AdvMap &advmap) {
    ...
    if (ps->info.stats.reported_epoch + ps->cct->_conf->osd_pg_stat_report_interval_max <
        advmap.osdmap->get_epoch()) {
        psdout(20) << "reporting stats to osd after "
                   << (advmap.osdmap->get_epoch() - ps->info.stats.reported_epoch) << " epochs"
                   << dendl;
        need_publish = true;
    }
...
boost::statechart::result PeeringState::ReplicaActive::react(const MLease &l) {
    ...
    pl->send_cluster_message(
        ps->get_primary().osd,
        make_message<MOSDPGLeaseAck>(
            epoch, spg_t(spgid.pgid, ps->get_primary().shard), ps->get_lease_ack()),
        epoch);
    return discard_event();
}
...
boost::statechart::result PeeringState::WaitActingChange::react(const AdvMap &advmap) {
    ...
    psdout(10) << "verifying no want_acting " << ps->want_acting << " targets didn't go down"
               << dendl;
...
void PeeringState::PeeringMachine::log_exit(const char *state_name, utime_t enter_time) {
    DECLARE_LOCALS;
    utime_t dur = ceph_clock_now() - enter_time;
    psdout(5) << "exit " << state_name << " " << dur << " " << event_count << " " << event_time
              << dendl;
