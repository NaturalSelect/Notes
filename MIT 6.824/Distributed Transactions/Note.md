...
*NOTE：由于分布式系统限制，不能像单机那样使用启动时间戳来发布写入，而是应该使用提交时间戳。*

*NOTE：这是为了确保写入严格位于读取之后（通过锁保证）。*

*NOTE：防止出现读取不一致快照，分区A完成了前一个事务提交，但是分区B还没有。*
...
*NOTE: 在此处显示的隔离级别（读写事务）是可串行化。*

*NOTE：我认为spanner的事务隔离级别是可串行化。*
...
*NOTE：推测必须与Percolator一样等待写锁释放，才能继续读取否则可能读到不一致的快照。*

*NOTE：分区A完成了前一个事务提交，但是分区B还没有。*
